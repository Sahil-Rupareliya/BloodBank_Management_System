<%-- 
    Document   : mpatient
    Created on : Mar 24, 2023, 8:34:44 AM
    Author     : CHINTAN
--%>
...
PreparedStatement countStmt = conn.prepareStatement("SELECT COUNT(*) FROM patient_reg");
...
PreparedStatement stmt = conn.prepareStatement("SELECT * FROM patient_reg LIMIT ?, ?");
stmt.setInt(1, start);
		...
		<div class="table">
			...
			<table class="intable" border="3px solid black" padding="10px">
				...
				<tr>
					...
					<td><%=resultSet.getString("nearestbloodbank")%></td>

					<td><%=resultSet.getString("mobileno")%></td>

					<td><a
						href="editpatient.jsp?patient_id=<%=resultSet.getString("patient_id")%>"><button
								class="btn-edit">
								<i class="fas fa-pencil-alt"></i>Edit</button></a></td>
					<td><a
						href="deletepatient.jsp?patient_id=<%=resultSet.getString("patient_id")%>"><button
								class="btn-delete">
