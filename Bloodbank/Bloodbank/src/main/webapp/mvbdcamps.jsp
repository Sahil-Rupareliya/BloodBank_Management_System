...
<input type="hidden" id="status"
	...
	<div class="name">Welcome&nbsp;&nbsp;Admin!!&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp; Volunteer Blood Camp</div>
...
PreparedStatement countStmt = conn.prepareStatement("SELECT COUNT(*) FROM vbdcamps");
...
PreparedStatement stmt = conn.prepareStatement("SELECT * FROM vbdcamps LIMIT ?, ?");
stmt.setInt(1, start);
...
%>
		...
		<div class="table">
			<div class="table-r1"></div>
			<table class="intable" border="3px solid black" padding="8px">
				<tr>
					<th>SR.NO</th>
				...
				<tr>
					...
					<td><a
						href="editvbd.jsp?campid=<%=resultSet.getString("campid")%>"><button
								class="btn-edit">
								<i class="fas fa-pencil-alt"></i>Edit</button></a></td>
					<td><a
						href="deletevbd.jsp?campid=<%=resultSet.getString("campid")%>"><button
								class="btn-delete">
								<i class="fas fa-trash-alt"></i>Delete</button></a></td>
                   <td><a href="resultvbd.jsp?id=<%=resultSet.getString("campid") %>"><button style="background-color:green;"type="button"class="btn-delete">        <i class="fas fa-envelope-open-text"></i>
                   RESULT</button></a></td>
					                  <td><a href="sendvbdstatus.jsp?campid=<%=resultSet.getString("campid") %>"><button type="button"  class="btn-delete"> <i class="fas fa-info-circle"></i>SEND</button></a></td>
...
<% } %>
	...
	<script type="text/javascript">
		...
		if (status == "success1") {
			swal("Congrats", "entry  updated Successfully", "success");
		}
		if (status == "success") {
			swal("Congrats", "entry  Delete Successfully", "success");
		}
	</script>
	 <script>
